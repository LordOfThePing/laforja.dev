...
- Cloudflare Tunnel para toda la app — pendiente — **siguiente tarea**. Decisiones ya tomadas:
  ...
  - Public hostnames a configurar en el panel del tunnel: `academia.flynnpedroa.engineer` path `/api/*` → `http://api:4000`, path `/webhooks/*` → `http://api:4000`, resto → `http://web:3000`.
